...
1. 在專案開發機編譯 JAR：
   ...
   ./gradlew bootJar
   ```
2. 將 `build/libs/nexusbot.jar`（`build.gradle.kts` 已固定檔名）與 `docs/docker/` 下的四個檔案一同複製至部署主機目錄
...
```bash
scripts/restart_k8s.bat
```

### 啟動 AIClient-2-API

```bash
scripts/start_deploy_aiclient.bat
```

### 啟動 ngrok + 更新 LINE Webhook

1. 建立 `scripts/deploy_ngrok/.env`（只需一次）：
   ```bash
   cp scripts/deploy_ngrok/.env.example scripts/deploy_ngrok/.env
   # 填入 NGROK_AUTHTOKEN 與 LINE_CHANNEL_ACCESS_TOKEN
   ```

2. 每次啟動：
   ```bash
   scripts/ngrok-tunnel.bat
   ```
   腳本自動完成：啟動 ngrok → 更新 K8s 環境變數 → 更新 LINE Webhook

> `ERR_NGROK_108`：tunnel 數量已滿，請至 https://dashboard.ngrok.com/agents 關閉舊 session。

### 觀測性（選用）

```bash
kubectl apply -f k8s/tempo/deployment.yaml -n acenexus    # Grafana Tempo
kubectl apply -f k8s/grafana/deployment.yaml -n acenexus  # Grafana UI（http://localhost:3000）
