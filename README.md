...
ACID 是資料庫交易（Transaction）必須遵守的四大特性，用來確保資料正確性與一致性：

| 特性                       | 說明                                                      |
|--------------------------|---------------------------------------------------------|
| **A - Atomicity（原子性）**   | 一筆交易中的所有操作要全做或全不做。<br/>如果中間失敗，會自動回滾（Rollback），不會留下部分結果。 |
| **C - Consistency（一致性）** | 交易前後資料庫都要維持合法狀態。<br/>不能因交易而破壞資料的完整性（例如違反外鍵、唯一值等限制）。     |
| **I - Isolation（隔離性）**   | 多筆交易同時執行時，彼此要互不干擾。<br/>就像是每筆交易「單獨」執行，避免髒讀、幻讀等問題。        |
| **D - Durability（持久性）**  | 一旦交易提交成功（Commit），即使系統當機或斷電，資料也會永久保留。                    |
