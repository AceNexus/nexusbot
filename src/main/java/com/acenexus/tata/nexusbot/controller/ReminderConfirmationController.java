...
public class ReminderConfirmationController {
    ...
    public String confirmReminder(
        ...
        ConfirmationResult result = reminderLogService.confirmByToken(token);
        ...
        model.addAttribute("alreadyConfirmed", result.alreadyConfirmed());

        if (result.success()) {
            if (result.alreadyConfirmed()) {
                logger.info("Reminder already confirmed: token={}", token);
            } else {
                logger.info("Reminder confirmed successfully: token={}", token);
            }
        } else {
            logger.warn("Confirmation failed for token {}: {}", token, result.message());
        }
