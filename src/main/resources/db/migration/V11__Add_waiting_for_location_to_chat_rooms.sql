...
ALTER TABLE chat_rooms ADD COLUMN waiting_for_location BOOLEAN NOT NULL DEFAULT FALSE;

-- 新增索引以優化查詢效能（可選但建議）
CREATE INDEX idx_chat_rooms_waiting_for_location ON chat_rooms (waiting_for_location);

-- 欄位註解說明用途
COMMENT ON COLUMN chat_rooms.waiting_for_location IS '標示聊天室是否正在等待用戶位置以進行廁所搜尋';